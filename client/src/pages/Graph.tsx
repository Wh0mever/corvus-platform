...
export default function Graph() {
  ...
  const buildGraph = useCallback(() => {
    ...
    const edges: GraphEdge[] = data.edges.filter(
      (e) => idSet.has(e.source as number) && idSet.has(e.target as number),
    );
    ...
    const node = g.append('g').selectAll<SVGGElement, GraphNode>('g')
      ...
      .on('click', (_, d) => {
        if (d.entity_type === 'contract') navigate(`/contracts/${d.entity_id}`);
      })
      .on('mouseenter', (e, d) => {
    ...
    node.append('circle')
      .attr('r', (d) => d.entity_type === 'contract' ? 12 : d.entity_type === 'company' ? 16 : 11)
      .attr('fill', (d) => getRiskColor(d.risk_score) + '22')
      .attr('stroke', (d) => getRiskColor(d.risk_score))
      .attr('stroke-width', (d) => d.risk_score >= 70 ? 2.5 : 1.5);

    node.append('text')
      .attr('text-anchor', 'middle').attr('dy', '0.35em')
      .attr('font-size', (d) => d.entity_type === 'company' ? 8 : 7)
    ...
    // Type icon below
    node.append('text')
      .attr('text-anchor', 'middle').attr('dy', (d) => (d.entity_type === 'contract' ? 12 : d.entity_type === 'company' ? 16 : 11) + 10)
      .attr('font-size', 7).attr('fill', 'var(--text-3)')
      .attr('pointer-events', 'none')
      .text((d) => d.entity_type === 'contract' ? '📄' : d.entity_type === 'company' ? '🏢' : '👤');
      ...
      <div style={{ flex: 1, position: 'relative', background: 'var(--card)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', overflow: 'hidden' }}>
        ...
        {tooltip && (
          <div style={{
            ...
            <div style={{ fontWeight: 600, color: 'var(--text-1)', marginBottom: 4 }}>{tooltip.node.label}</div>
            <div style={{ color: 'var(--text-3)', fontSize: 10, marginBottom: 3 }}>
              {tooltip.node.entity_type === 'contract' ? 'Контракт' : tooltip.node.entity_type === 'company' ? 'Компания' : 'Персона'}
              {' · '}
              <span style={{ color: getRiskColor(tooltip.node.risk_score) }}>риск {tooltip.node.risk_score}</span>
            </div>
            {tooltip.node.entity_type === 'contract' && (
