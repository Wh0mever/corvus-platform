...
export default function AIAnalyst() {
  ...
  // Remove the duplicate user message added by onSuccess if we pre-add it
  // Actually let's not pre-add — just add in onSuccess
  const sendClean = (text: string) => {
    if (!text.trim() || isPending) return;
    setInput('');
    ask(text);
  };
    ...
    <div className="page-inner" style={{ display: 'flex', flexDirection: 'column', height: '100%', maxWidth: 860, margin: '0 auto', width: '100%' }}>
        ...
        {messages.map((msg, i) => (
          <div key={i} style={{
            ...
            <div style={{
              ...
              {msg.role === 'user'
                ? <span style={{ color: 'var(--text-1)', lineHeight: 1.5 }}>{msg.text}</span>
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
        <input
          ...
          onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && sendClean(input)}
          disabled={isPending}
        />
        <button
          className="btn-primary"
          onClick={() => sendClean(input)}
